...
exports.read = function (root, files, progress, callback,exifBinaryPath) {
  if (files.length === 0) {
  ...
  const spawnPath=(exifBinaryPath ? path.join(exifBinaryPath,'exiftool') :'exiftool');
  const child = childProcess.spawn(spawnPath, args, {
    cwd: root,
